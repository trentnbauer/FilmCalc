...
3. Decide whether to auto-merge or leave it for review: for every finding whose `field` is a price-like number (not `source`, not a field going from empty to a value), compute the absolute percent change between `old` and `new` (skip any finding where `old` is empty/zero — nothing to divide by, treat it as a review-worthy addition instead of a swing).
   ...
   - If **any single finding swings by more than 30%**, it's more likely a misread (wrong pack size, wrong tier, a stray digit) than a genuine price move — don't just take your own earlier read's word for it a second time. This repo already runs an independent reviewer on every PR touching `films/**`/`labs/**` — the `claude-sanity-check` job in `validate data.yml`, a *separate* Claude invocation with no memory of how you arrived at this number, which re-fetches any linked page itself and sanity-checks the price against it. Wait for its comment (it runs automatically once this PR's other checks pass — poll with `gh pr view <number> --json comments`, checking every ~20s for up to about 5 minutes) and read it:
     - If it comes back with no concern about the swung item(s), merge it yourself — `gh pr merge --squash --auto --delete-branch`. This is now independently corroborated, not just self-certified.
     - If it raises a concern about the swung item(s), or if it never comments within the wait (its own run may itself be genuinely inconclusive, blocked, or slow) — don't merge. Leave the PR open for the repo owner, quoting the sanity-check's concern (or noting it never came back) and naming exactly which finding(s) triggered the hold in the PR body — e.g. "Kodak Portra 400 devCost $12 → $19, a 58% jump — flagging for a human glance instead of auto-merging."
